...
- `funding_YYYY-MM.csv` — фандинг XAU/PAXG/XAG/BTC/ETH × 4 площадки, 10 мин (`src/record_funding.py`): `ts_utc,exchange,symbol,market_id,rate,rate_bp` (единицы сырые — не смешивать без конверсии; BTC/ETH с 18.09).
- `funding_binance_{paxg,btc,eth}_hist.csv` — история фандинга Binance с 26.06 (PAXG) / 05.04 (BTC,ETH), тот же формат.
